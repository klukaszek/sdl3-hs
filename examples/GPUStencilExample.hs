...
import Data.Foldable (for_, asum, find)
import Data.IORef
...
-- ... (defaultShaderCreateInfo, defaultColorTargetBlendState, etc. from previous example)
-- We will define specific DepthStencilStates below.
  ...
  where
    ...
    findDepthStencilFormat dev = do
        let usage = SDL_GPU_TEXTUREUSAGE_DEPTH_STENCIL_TARGET
            ...
            formatsToCheck = [ SDL_GPU_TEXTUREFORMAT_D24_UNORM_S8_UINT
    ...
    createPipelines dev win vertShader fragShader depthStencilFormat = do
        ...
        let maskerStencilOpState = SDLGPUStencilOpState
            ...
            maskerDepthStencilState = SDLGPUDepthStencilState
        ...
        case (mMaskerPipe, mMaskeePipe) of
            ...
            (p1, p2)           -> do -- Cleanup partially created pipelines
...
calculateVertexDataSize dataList = do
    ...
    sdlLog $ printf "Vertex Info - SizeOf: %d, Count: %d, Total Bytes (CSize): %d, Total Bytes (Word32): %d"
                    vertexSize numVertices (fromIntegral totalCSize :: Int) totalSizeWord32
...
mapAndCopyData dev tb dataList = do
    bracket (sdlMapGPUTransferBuffer dev tb False)
            (\mPtr -> when (isJust mPtr) $ sdlUnmapGPUTransferBuffer dev tb) $ \case
...
handleEventGPU context event _ = case event of
    ...
    case maybeCmdbuf of
        ...
        Just cmdbuf -> do
            ...
            case maybeSwapchain of
                ...
                Just (swapchainTexture, _, _) -> do -- Don't need width/height here
                    -- Define Color Target
                    ...
                    let depthStencilTargetInfo = SDLGPUDepthStencilTargetInfo
                    ...
                    bracket (sdlBeginGPURenderPass cmdbuf [colorTargetInfo] (Just depthStencilTargetInfo))

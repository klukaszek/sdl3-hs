...
runAppGPU context@Context {..} = do
  ...
  bracket
    ...
    $ \case
      ...
      Just _resources -> do
        -- Resources used directly in Draw now
...
createResources Context {contextDevice = dev, contextWindow = win} = do
  ...
  case (maybeGfxVertShader, maybeGfxFragShader, maybeFillTexturePipeline) of
      ...
      case maybeDims of
        ...
        Just (wInt, hInt) -> do
          ...
          bracketOnError
            ...
            $ \maybeInitialCmdBuf ->
              bracketOnError
                (createTransferBuffer dev vertexDataSize SDL_GPU_TRANSFERBUFFERUSAGE_UPLOAD "VBTransfer")
    ...
    _ -> do
      -- Shader or compute pipeline loading failed
...
defaultSamplerCreateInfoRepeat =

...
import Control.Monad (unless, when, void, forM, forM_) -- Add forM/forM_
...
import Foreign.C.Types (CFloat, CSize, CInt) -- Add CInt
...
import Data.Bits ((.|.))
import Data.Text (Text) -- Import Text for title setting
import qualified Data.Text as T

-- 1. Define Vertex Structure and Storable Instance (Same as before)
...
vertexDataCW = -- Standard winding order (matches previous example)
...
vertexDataCCW = -- Reversed winding order
...
-- Reuse default states (copy-pasted for completeness, ensure GPUCommon is up-to-date if modified)
...
defaultVertexInputState = SDLGPUVertexInputState { inputVertexBuffers = [], inputVertexAttribs = [] } -- Used for RawTriangle, adapt for VB
...
runAppGPU context@Context{..} = do
  ...
  where
    ...
    createResources ctx@Context{ contextDevice = dev, contextWindow = win } = do
        ...
        bracketOnError (loadShader dev "PositionColor.vert" SDL_GPU_SHADERSTAGE_VERTEX defaultShaderCreateInfo)
                       (maybe (return ()) (sdlReleaseGPUShader dev)) $ \maybeVertShader ->
         bracketOnError (loadShader dev "SolidColor.frag" SDL_GPU_SHADERSTAGE_FRAGMENT defaultShaderCreateInfo)
                        (maybe (return ()) (sdlReleaseGPUShader dev)) $ \maybeFragShader -> do
    ...
    createPipelines dev win vertShader fragShader = do
        ...
        let colorTargetDesc = SDLGPUColorTargetDescription swapchainFormat defaultColorTargetBlendState
            ...
            basePipelineCI = SDLGPUGraphicsPipelineCreateInfo
                ...
                , multisampleState = defaultMultiSampleState, rasterizerState = defaultRasterizerState -- Will be modified
                , targetInfo = targetInfo, depthStencilState = defaultDepthStencilState
                , props = 0
                }
    ...
    createAndUploadVertexBuffers dev dataCW dataCCW = do
        ...
        if null dataCW || null dataCCW then do
            ...
            bracketOnError (createBufferPair dev (fromIntegral totalCSizeCW) (fromIntegral totalCSizeCCW))
                           (cleanupBufferPair dev) $ \maybeBuffers -> do
      ...
      where
        ...
        uploadBothViaTransferBuffer dev vbCW vbCCW dataCW dataCCW transferSize totalSizeCW_CSize totalSizeCW_W32 totalSizeCCW_W32 = do
            bracket (createTransferBuffer dev transferSize)
        ...
        mapAndCopyBothData dev tb dCW dCCW offsetBytesCW_CSize = do
             bracket (sdlMapGPUTransferBuffer dev tb False)
                     (\mPtr -> when (isJust mPtr) $ sdlUnmapGPUTransferBuffer dev tb) $ \maybeMappedPtr -> do
        ...
        uploadBothDataCommandBuffer dev vbCW vbCCW tb sizeCW sizeCCW = do
            bracket (sdlAcquireGPUCommandBuffer dev) cleanupCommandBuffer $ \maybeCmdBuf -> do
                case maybeCmdBuf of
                    ...
                    Just cmdBuf ->
                        bracket (sdlBeginGPUCopyPass cmdBuf) cleanupCopyPass $ \maybeCopyPass -> do
...
renderFrameGPU Context{..} AppResources{..} AppState{..} useNonBlockingAcquire = do
    ...
    -- Update window title with current mode (optional, but helpful)
    -- Can be slow if done every frame, but ok for this example
    -- SDL.windowTitle contextWindow $= T.pack ("SDL3 Haskell GPU Cull Mode - Mode: " ++ (modeNames !! currentMode))

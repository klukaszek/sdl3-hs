...
runAppGPU context = do
    ...
    bracket (createAndTestResources context)
...
createAndTestResources context@Context{..} = do
    ...
    bracketOnError (loadImage ("Content" </> "Images" </> "ravioli.bmp"))
        (\case Just surf -> sdlDestroySurface surf; _ -> pure ()) $ \case
            ...
            Just surfacePtr ->
                ...
                bracketOnError (pure surfacePtr) (\_ -> sdlLog "Releasing loaded surface from inner bracket" >> sdlDestroySurface surfacePtr) $ \surf -> do
                    ...
                    maybeTexSmall <- sdlCreateGPUTexture contextDevice
                    ...
                    (_, bufferDataSizeC, bufferDataSizeW32) <- calculateBufferDataSize bufferData "OriginalBufferData"
                    ...
                    case (maybeOrigTex, maybeTexCopy, maybeTexSmall, maybeOrigBuf, maybeBufCopy, maybeUploadTransfer, maybeDownloadTransfer) of
                        (Just oTex, Just tCopy, Just tSmall, Just oBuf, Just bCopy, Just upTrans, Just downTrans) -> do
                            mapAndPokeOk <- bracket (sdlMapGPUTransferBuffer contextDevice upTrans False)
                            ...
                            if not mapAndPokeOk then do
                                ...
                                case mCmdBuf of
                                    ...
                                    Just cmdBuf -> do
                                        ...
                                        -- First Copy Pass
                                        validAfterCopyPass1 <- do -- Introduce a sub-do block to capture the Bool
                                        ...
                                        if not validAfterCopyPass1 then
                                                ...
                                                Just cp2 -> do
                                                    ...
                                                    case mFence of
                                                        ...
                                                        Just fence -> do
                                                            ...
                                                            if waitOk then
                                                                bracket (sdlMapGPUTransferBuffer contextDevice downTrans False)
                                                                    ...
                                                                    $ \case
                                                                        ...
                                                                        Just mappedDownPtr -> do
                                                                            ...
                                                                            if texMatch then sdlLog "SUCCESS! Texture data matches."
                        ...
                        _ -> do -- Case for one of the initial Just oTex ... downTrans being Nothing
    ...
    case maybeCmdbuf of
        ...
        Just cmdbuf -> do
            ...
            case maybeSwapResult of
                ...
                Just (swapchainTexture, swapW, swapH) -> do -- swapW, swapH are Word32

                    -- Clear swapchain
                    let colorTargetInfo = defaultColorTargetInfo -- Use default
                    ...
                    let blitInfoOrig = SDLGPUBlitInfo
                            ...
                            SDL_GPU_LOADOP_LOAD (SDLFColor 0 0 0 0) SDL_FLIP_NONE SDL_GPU_FILTER_NEAREST False
                    sdlBlitGPUTexture cmdbuf blitInfoOrig

                    -- Blit TextureCopy to top-right quadrant
                    let blitInfoCopy = SDLGPUBlitInfo
                    ...
                    let blitInfoSmall = SDLGPUBlitInfo
                            ...
                            ((defaultBlitRegion swapchainTexture (fromIntegral swapW) (fromIntegral swapH))
                                { gpuBlitRegX = swapW `div` 4, gpuBlitRegY = swapH `div` 2
                                , gpuBlitRegW = swapW `div` 2, gpuBlitRegH = swapH `div` 2
                                })
                            SDL_GPU_LOADOP_LOAD (SDLFColor 0 0 0 0) SDL_FLIP_NONE SDL_GPU_FILTER_NEAREST False

...
runAppGPU context = do
  ...
  where
      ...
      bracketOnError (loadShaders dev) (releaseShaders dev) $ \case
        ...
        Just (vertShader, fragShader) -> do
          ...
          bracketOnError
            ...
            (maybe (return ()) (sdlReleaseGPUGraphicsPipeline dev))
            $ \case
    ...
    uploadBufferData dev vertexBuffer indexBuffer vertices indices = do
      ...
      bracket
        (createTransferBuffer dev totalTransferSize SDL_GPU_TRANSFERBUFFERUSAGE_UPLOAD "transfer")
        (cleanupTransferBuffer dev)
        $ \case
          Nothing -> return False -- Transfer buffer creation failed
          Just transferBuffer -> do
            -- Map, Poke, Unmap
            mapCopySuccess <- bracket
              (sdlMapGPUTransferBuffer dev transferBuffer False)
              (\mPtr -> when (isJust mPtr) $ sdlUnmapGPUTransferBuffer dev transferBuffer)
              $ \case
...
handleEventGPU _ event AppState {..} = case event of
    ...
    Just cmdbuf -> do
      ...
      case maybeSwapchain of
        ...
        Just (swapchainTexture, _, _) -> do
          ...
          -- 3. Begin Render Pass
          bracket
            (sdlBeginGPURenderPass cmdbuf [colorTargetInfo] Nothing) -- No depth/stencil

...
-- | Print a pre-formatted string to an SDL_IOStream using the C wrapper.
foreign import ccall unsafe "wrapper_SDL_IOprintf"
  c_sdlIOprintf :: Ptr SDLIOStream -> CString -> IO CSize

sdlIOprintf :: Ptr SDLIOStream -> String -> IO CSize
sdlIOprintf stream str =
  withCString str $ \cstr ->
    c_sdlIOprintf stream cstr

-- | Foreign import for SDL_IOvprintf (va_list version)
foreign import ccall unsafe "SDL_IOvprintf"
  sdlIOvprintfRaw :: Ptr SDLIOStream -> CString -> Ptr () -> IO CSize

-- | Haskell wrapper for SDL_IOvprintf.
-- Note: There is no direct way to construct a va_list in Haskell, so this is only useful
-- if you have a C helper to construct the va_list. For most use-cases, format in Haskell and use sdlWriteIO.
sdlIOvprintf :: Ptr SDLIOStream -> String -> Ptr () -> IO CSize
sdlIOvprintf stream format vaListPtr =
  withCString format $ \cstr ->
    sdlIOvprintfRaw stream cstr vaListPtr
...
sdlLoadFile file =
  alloca $ \(sizePtr :: Ptr CSize) ->
    -- Allocate space for the size output parameter
    withCString file $ \cfile -> do
      -- Convert FilePath to CString

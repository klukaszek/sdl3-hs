...
import Foreign (Ptr, Word32, Word64, alloca, nullFunPtr, peek, peekArray)
...
foreign import ccall unsafe "SDL_Vulkan_LoadLibrary" c_sdlVulkanLoadLibrary :: CString -> IO CBool

-- | Dynamically load the Vulkan loader library.
sdlVulkanLoadLibrary :: String -> IO Bool
sdlVulkanLoadLibrary path = withCString path (fmap fromCBool . c_sdlVulkanLoadLibrary)
...
sdlVulkanUnloadLibrary = c_sdlVulkanUnloadLibrary :: IO ()
...
sdlVulkanGetInstanceExtensions = alloca \pCount -> do
...
sdlVulkanCreateSurface (SDLWindow winPtr) inst pAllocationCallbacks = alloca \ptrSurfaceKHR -> do

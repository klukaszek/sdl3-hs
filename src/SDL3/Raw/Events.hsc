...
import SDL3.Raw.Video (SDLWindow(..), SDLWindowID)
import SDL3.Raw.Mouse (SDLMouseID, SDLMouseButtonFlags)
import SDL3.Raw.Joystick (SDLJoystickID)
import SDL3.Raw.Gamepad (SDLGamepadButton)
import SDL3.Raw.Touch (SDLTouchID, SDLFingerID)
import SDL3.Raw.Pen (SDLPenID)
import SDL3.Raw.Keyboard (SDLKeyboardID)
...
pattern SDL_ADDEVENT = (#const SDL_ADDEVENT) :: SDLEventAction  
pattern SDL_PEEKEVENT = (#const SDL_PEEKEVENT) :: SDLEventAction
pattern SDL_GETEVENT = (#const SDL_GETEVENT) :: SDLEventAction

-- Pattern synonyms for SDLEventType, mapping to C constants
pattern SDL_EVENT_FIRST                   = (#const SDL_EVENT_FIRST) :: SDLEventType
pattern SDL_EVENT_QUIT                    = (#const SDL_EVENT_QUIT) :: SDLEventType
pattern SDL_EVENT_TERMINATING             = (#const SDL_EVENT_TERMINATING) :: SDLEventType
pattern SDL_EVENT_LOW_MEMORY              = (#const SDL_EVENT_LOW_MEMORY) :: SDLEventType
pattern SDL_EVENT_WILL_ENTER_BACKGROUND   = (#const SDL_EVENT_WILL_ENTER_BACKGROUND) :: SDLEventType
pattern SDL_EVENT_DID_ENTER_BACKGROUND    = (#const SDL_EVENT_DID_ENTER_BACKGROUND) :: SDLEventType
pattern SDL_EVENT_WILL_ENTER_FOREGROUND   = (#const SDL_EVENT_WILL_ENTER_FOREGROUND) :: SDLEventType
pattern SDL_EVENT_DID_ENTER_FOREGROUND    = (#const SDL_EVENT_DID_ENTER_FOREGROUND) :: SDLEventType
pattern SDL_EVENT_LOCALE_CHANGED          = (#const SDL_EVENT_LOCALE_CHANGED) :: SDLEventType
pattern SDL_EVENT_SYSTEM_THEME_CHANGED    = (#const SDL_EVENT_SYSTEM_THEME_CHANGED) :: SDLEventType
pattern SDL_EVENT_DISPLAY_ORIENTATION     = (#const SDL_EVENT_DISPLAY_ORIENTATION) :: SDLEventType
pattern SDL_EVENT_DISPLAY_ADDED           = (#const SDL_EVENT_DISPLAY_ADDED) :: SDLEventType
pattern SDL_EVENT_DISPLAY_REMOVED         = (#const SDL_EVENT_DISPLAY_REMOVED) :: SDLEventType
pattern SDL_EVENT_DISPLAY_MOVED           = (#const SDL_EVENT_DISPLAY_MOVED) :: SDLEventType
...
pattern SDL_EVENT_WINDOW_MOVED            = (#const SDL_EVENT_WINDOW_MOVED) :: SDLEventType
pattern SDL_EVENT_WINDOW_RESIZED          = (#const SDL_EVENT_WINDOW_RESIZED) :: SDLEventType
pattern SDL_EVENT_WINDOW_PIXEL_SIZE_CHANGED = (#const SDL_EVENT_WINDOW_PIXEL_SIZE_CHANGED) :: SDLEventType
pattern SDL_EVENT_WINDOW_MINIMIZED        = (#const SDL_EVENT_WINDOW_MINIMIZED) :: SDLEventType
pattern SDL_EVENT_WINDOW_MAXIMIZED        = (#const SDL_EVENT_WINDOW_MAXIMIZED) :: SDLEventType
pattern SDL_EVENT_WINDOW_RESTORED         = (#const SDL_EVENT_WINDOW_RESTORED) :: SDLEventType
pattern SDL_EVENT_WINDOW_MOUSE_ENTER      = (#const SDL_EVENT_WINDOW_MOUSE_ENTER) :: SDLEventType
pattern SDL_EVENT_WINDOW_MOUSE_LEAVE      = (#const SDL_EVENT_WINDOW_MOUSE_LEAVE) :: SDLEventType
pattern SDL_EVENT_WINDOW_FOCUS_GAINED     = (#const SDL_EVENT_WINDOW_FOCUS_GAINED) :: SDLEventType
pattern SDL_EVENT_WINDOW_FOCUS_LOST       = (#const SDL_EVENT_WINDOW_FOCUS_LOST) :: SDLEventType
pattern SDL_EVENT_WINDOW_CLOSE_REQUESTED  = (#const SDL_EVENT_WINDOW_CLOSE_REQUESTED) :: SDLEventType
pattern SDL_EVENT_WINDOW_DISPLAY_SCALE_CHANGED = (#const SDL_EVENT_WINDOW_DISPLAY_SCALE_CHANGED) :: SDLEventType
pattern SDL_EVENT_WINDOW_DESTROYED        = (#const SDL_EVENT_WINDOW_DESTROYED) :: SDLEventType
pattern SDL_EVENT_KEY_DOWN                = (#const SDL_EVENT_KEY_DOWN) :: SDLEventType
...
pattern SDL_EVENT_MOUSE_MOTION            = (#const SDL_EVENT_MOUSE_MOTION) :: SDLEventType
pattern SDL_EVENT_MOUSE_BUTTON_DOWN       = (#const SDL_EVENT_MOUSE_BUTTON_DOWN) :: SDLEventType
pattern SDL_EVENT_MOUSE_BUTTON_UP         = (#const SDL_EVENT_MOUSE_BUTTON_UP) :: SDLEventType
...
pattern SDL_EVENT_JOYSTICK_ADDED          = (#const SDL_EVENT_JOYSTICK_ADDED) :: SDLEventType
pattern SDL_EVENT_JOYSTICK_REMOVED        = (#const SDL_EVENT_JOYSTICK_REMOVED) :: SDLEventType
pattern SDL_EVENT_GAMEPAD_AXIS_MOTION     = (#const SDL_EVENT_GAMEPAD_AXIS_MOTION) :: SDLEventType
pattern SDL_EVENT_GAMEPAD_BUTTON_DOWN     = (#const SDL_EVENT_GAMEPAD_BUTTON_DOWN) :: SDLEventType
pattern SDL_EVENT_GAMEPAD_BUTTON_UP       = (#const SDL_EVENT_GAMEPAD_BUTTON_UP) :: SDLEventType
pattern SDL_EVENT_GAMEPAD_ADDED           = (#const SDL_EVENT_GAMEPAD_ADDED) :: SDLEventType
pattern SDL_EVENT_GAMEPAD_REMOVED         = (#const SDL_EVENT_GAMEPAD_REMOVED) :: SDLEventType
pattern SDL_EVENT_GAMEPAD_REMAPPED        = (#const SDL_EVENT_GAMEPAD_REMAPPED) :: SDLEventType
pattern SDL_EVENT_GAMEPAD_TOUCHPAD_DOWN   = (#const SDL_EVENT_GAMEPAD_TOUCHPAD_DOWN) :: SDLEventType
pattern SDL_EVENT_GAMEPAD_TOUCHPAD_MOTION = (#const SDL_EVENT_GAMEPAD_TOUCHPAD_MOTION) :: SDLEventType
pattern SDL_EVENT_GAMEPAD_TOUCHPAD_UP     = (#const SDL_EVENT_GAMEPAD_TOUCHPAD_UP) :: SDLEventType
pattern SDL_EVENT_GAMEPAD_SENSOR_UPDATE   = (#const SDL_EVENT_GAMEPAD_SENSOR_UPDATE) :: SDLEventType
pattern SDL_EVENT_GAMEPAD_UPDATE_COMPLETE = (#const SDL_EVENT_GAMEPAD_UPDATE_COMPLETE) :: SDLEventType
pattern SDL_EVENT_GAMEPAD_STEAM_HANDLE_UPDATED = (#const SDL_EVENT_GAMEPAD_STEAM_HANDLE_UPDATED) :: SDLEventType
pattern SDL_EVENT_CLIPBOARD_UPDATE        = (#const SDL_EVENT_CLIPBOARD_UPDATE) :: SDLEventType
pattern SDL_EVENT_DROP_FILE               = (#const SDL_EVENT_DROP_FILE) :: SDLEventType
pattern SDL_EVENT_DROP_TEXT               = (#const SDL_EVENT_DROP_TEXT) :: SDLEventType
pattern SDL_EVENT_DROP_BEGIN              = (#const SDL_EVENT_DROP_BEGIN) :: SDLEventType
pattern SDL_EVENT_DROP_COMPLETE           = (#const SDL_EVENT_DROP_COMPLETE) :: SDLEventType
pattern SDL_EVENT_USER                    = (#const SDL_EVENT_USER) :: SDLEventType
pattern SDL_EVENT_LAST                    = (#const SDL_EVENT_LAST) :: SDLEventType
...
sdlSetEventFilter :: FunPtr SDLEventFilter -> Ptr () -> IO ()
sdlSetEventFilter = sdlSetEventFilterRaw
...
sdlGetEventFilter :: IO (Maybe (FunPtr SDLEventFilter, Ptr ()))
sdlGetEventFilter = alloca $ \filterPtr -> alloca $ \userdataPtr -> do
  ...
  if success then do
    ...
    return $ Just (filt, userdata)
  else return Nothing

foreign import ccall "SDL_AddEventWatch"
  sdlAddEventWatchRaw :: FunPtr SDLEventFilter -> Ptr () -> IO CBool

sdlAddEventWatch :: FunPtr SDLEventFilter -> Ptr () -> IO Bool
sdlAddEventWatch filtPtr userdata =
  toBool <$> sdlAddEventWatchRaw filtPtr userdata

foreign import ccall "SDL_RemoveEventWatch"
  sdlRemoveEventWatchRaw :: FunPtr SDLEventFilter -> Ptr () -> IO ()

sdlRemoveEventWatch :: FunPtr SDLEventFilter -> Ptr () -> IO ()
sdlRemoveEventWatch = sdlRemoveEventWatchRaw

foreign import ccall "SDL_FilterEvents"
  sdlFilterEventsRaw :: FunPtr SDLEventFilter -> Ptr () -> IO ()

sdlFilterEvents :: FunPtr SDLEventFilter -> Ptr () -> IO ()
sdlFilterEvents = sdlFilterEventsRaw
